...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        self.log_name = opt.checkpoints_dir # os.path.join(opt.checkpoints_dir, opt.name, 'loss_log.txt')
        with open("./training_log", "a") as log_file:
            now = time.strftime("%c")
        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    ...
                    images.append(image_numpy.transpose([0, 1]))
                    idx += 1
                    if idx % ncols == 0:
                ...
                try:
                    for label, image in visuals.items():
                        image_numpy = util.tensor2im(image)
                        self.vis.image(image_numpy.transpose([0, 1]), opts=dict(title=label),
