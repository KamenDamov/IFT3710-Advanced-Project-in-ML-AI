...
class AttentionGenerator(nn.Module):
    ...
    # forward method
    def forward(self, input):
        x = F.pad(input[0], (3, 3, 3, 3), 'reflect')
        x = F.relu(self.conv1_norm(self.conv1(x)))
        ...
        image = image if len(image.size())==4 else image.unsqueeze(0)
        image1 = image[:, 0:3, :, :]
        ...
        attention = softmax_(attention.unsqueeze(0)) if len(attention.size())==3 else softmax_(attention)
