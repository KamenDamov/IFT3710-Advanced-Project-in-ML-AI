...
def move_data(target_images, target_labels):
    os.makedirs(target_images, exist_ok=True)
    os.makedirs(target_labels, exist_ok=True)
    
    dataset = list(ds.DataSet("./data"))
...
def main():
    ...
    parser.add_argument("--smart_crop", default=False, type=bool)
    # Dataset parameters
    parser.add_argument(
        "--data_path",
    ...
    parser.add_argument(
        "--work_dir", default="./src/models/production/work_dir", help="path where to save models and logs"
    )
    ...
    if args.smart_crop:
        move_data(img_path, gt_path)

    img_names = sorted(os.listdir(img_path))
    gt_names = [img_name.split(".")[0] + ".png" for img_name in img_names]
    meta_names = [img_name.split(".")[0] + ".csv" for img_name in img_names]
    ...
    train_files = [
        {"img": join(img_path, img_names[i]), "label": join(gt_path, gt_names[i]), "meta": join(gt_path, meta_names[i])}
